...
export class TokenUtil {
  static async jwtSign(payload: { email: string; _id: string }, configService: ConfigService): Promise<string> {
    try {
      const signOptions = {
        ...
        encryptionKey = configService.get<string>('SESSION.CRYPTO_KEY') as string,
  ...
  static async jwtVerify(token: string, configService: ConfigService): Promise<Partial<I_USER>> {
    try {
      const signOptions = {
          algorithm: configService.get<string>('SESSION.ALGORITHM') as any,
          audience: configService.get<string>('SESSION.AUDIENCE') as string,
        ...
        tokenInfo = verify(decodedToken, readFileSync(publicKey, 'utf8'), signOptions) as I_USER;
  ...
  static async verifyUser(token: string, configService: ConfigService): Promise<Partial<I_USER>> {
